#!/bin/bash
# DSKDv2 + IMPACT (ALP-KD 1-all): OPT-2.7B (LoRA) <- Qwen2.5-7B-Instruct (LoRA) — HP aligned with scripts/gptxl/
GPUS=(1)
...
SETTING=criterion=${CRITERION}__${CONFIG}__teacher=${TEACHER_MODEL_NAME}__align=${CHOOSE_ALIGN}__kd^rate=${KD_RATE}__kd^temp=${KD_TEMP}__impact^λ=${IMPACT_LAMBDA}__impact^k=${IMPACT_TOP_K}__epoch=${EPOCH}__bsz=${BATCH_SIZE}x${GRAD_ACC}x${GPUS_PER_NODE}=$((BATCH_SIZE * GRAD_ACC * GPUS_PER_NODE * NNODES))__lr=${LR}__proj^lr=${PROJECTOR_LR}
...
OPTS+=" --impact-choose-layer ${CHOOSE_LAYER}"
