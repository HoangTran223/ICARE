#!/bin/bash
# DSKDv2 + IMPACT with RAIL_KD alignment (GLMKD): choose_align=1-random
# Random sorted teacher layers, 1:1 with student layers; resample each epoch by default.
GPUS=(0)
...
SETTING=criterion=${CRITERION}__${CONFIG}__teacher=${TEACHER_MODEL_NAME}__align=${CHOOSE_ALIGN}__rail^ep=${RAIL_KD_EPOCHS}__kd^rate=${KD_RATE}__kd^temp=${KD_TEMP}__impact^λ=${IMPACT_LAMBDA}__impact^k=${IMPACT_TOP_K}__epoch=${EPOCH}__bsz=${BATCH_SIZE}x${GRAD_ACC}x${GPUS_PER_NODE}=$((BATCH_SIZE * GRAD_ACC * GPUS_PER_NODE * NNODES))__lr=${LR}__proj^lr=${PROJECTOR_LR}
...
OPTS+=" --impact-choose-align ${CHOOSE_ALIGN}"
OPTS+=" --impact-rail-kd-epochs ${RAIL_KD_EPOCHS}"
OPTS+=" --impact-rail-kd-show-layers"
