#!/bin/bash
# ALM + IMPACT — Qwen1.5-1.8B → GPT2-120M on Dolly
#
# Training (ALM backbone): ARTINT-S-26-00444 (WARD), Table 2 — GPT2-120M column
#   Epoch 20 | LR 5e-4 | batch 8 | cosine | full fine-tune | KD temperature τ=2.0
# Eval (§5.1): ROUGE-L; sampling T=1, top-p=1; train seed 10.
#
# ALM (tokenkit): τ_bin=100, bias=0.1, unconstrained, merge_by_space_prob+append_space,
#   GradMag preserve_mag.
# IMPACT: ICARE extension (not in WARD paper); defaults below pair with batch 8 / τ=2.
GPUS=(1)
export CUDA_VISIBLE_DEVICES=$(IFS=,; echo "${GPUS[*]}")

MASTER_ADDR=localhost
MASTER_PORT=61$(($RANDOM%90+10))
NNODES=1
NODE_RANK=0
GPUS_PER_NODE=${#GPUS[@]}
...
CKPT_TYPE="gpt2"
# Paper: full FT on Dolly from pretrained GPT-2 120M (not SFT).
...
# ALM / tokenkit
ALM_BINARIZATION_TEMP=50.0
ALM_BIAS_THRESHOLD=0.02
ALM_MODE="merge_by_space_prob+append_space"
ALM_ALIGNMENT="unconstrained"
MULTITASK_AGG="approx_gradmag_preserve_mag"
ALM_LOSS_WEIGHT=3.0

# IMPACT (ICARE; GradMag multitask — impact_lambda often secondary when GradMag on)
IMPACT_LAMBDA=10
IMPACT_TOP_K=10
IMPACT_BI_TAU=2.0
...
SETTING=criterion=${CRITERION}__${CONFIG}__teacher=${TEACHER_MODEL_NAME}__alm^align=${ALM_ALIGNMENT}__agg=${MULTITASK_AGG}__alm^temp=${ALM_BINARIZATION_TEMP}__impact^k=${IMPACT_TOP_K}__epoch=${EPOCH}__bsz=${BATCH_SIZE}x${GRAD_ACC}x${GPUS_PER_NODE}=$((BATCH_SIZE * GRAD_ACC * GPUS_PER_NODE * NNODES))__lr=${LR}
