...
def add_hp_args(parser: argparse.ArgumentParser):
    ...
    # IMPACT hyperparameters (DSKDv2 + IMPACT; --ipact-* kept for legacy scripts)
    group.add_argument("--impact-lambda", "--ipact-lambda", dest="impact_lambda", type=float, default=1.0)
    group.add_argument("--impact-top-k", "--ipact-top-k", dest="impact_top_k", type=int, default=4)
    group.add_argument("--impact-bi-tau", "--ipact-bi-tau", dest="impact_bi_tau", type=float, default=1.0)
    group.add_argument("--impact-lambda-reg", "--ipact-lambda-reg", dest="impact_lambda_reg", type=float, default=1.0)
    group.add_argument(
        "--impact-choose-layer",
        "--choose-layer",
        ...
        help="Teacher layer selection for IMPACT when choose_align=BI: BI (default), random, first, last, or PPL",
    )
    group.add_argument(
        "--impact-choose-align",
        "--choose-align",
