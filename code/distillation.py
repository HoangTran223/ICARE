...
def infer_resume_from_load(load_path, start_epoch, resume_global_step):
    """Infer start_epoch / resume_global_step from checkpoint dir name (epoch9_step6435_...)."""
    if not load_path:
        return start_epoch, resume_global_step
    base = os.path.basename(load_path.rstrip("/"))
...
def finetune(
    ...
    step = 0
    start_epoch_idx = max(0, getattr(args, "start_epoch", 1) - 1)
    resume_global_step = max(0, getattr(args, "resume_global_step", 0))
    logging_output = {
        "epoch": start_epoch_idx,
    ...
    if start_epoch_idx > 0:
        log_rank(
            "Resuming training from epoch {} (global_step={}, total_epochs={})".format(
                start_epoch_idx + 1, resume_global_step, args.num_epochs
            )
        )

    for epoch in range(start_epoch_idx, args.num_epochs):
...
def main():
    ...
    if args.load is not None:
        ...
        args.start_epoch, args.resume_global_step = infer_resume_from_load(
            args.load, args.start_epoch, args.resume_global_step
        )
        log_rank(
            "[INFO] Resume: start_epoch={} resume_global_step={}".format(
                args.start_epoch, args.resume_global_step
            )
        )

    if args.do_train and args.resume_global_step > 0:
        log_rank(
            "Fast-forwarding LR scheduler by {} steps (cosine, total_iters={})".format(
                args.resume_global_step, args.total_iters
            )
        )
